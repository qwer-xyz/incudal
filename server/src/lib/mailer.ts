...
export async function sendPasswordResetEmail(
    ...
    try {
        ...
        const paragraphs: string[] = [
            ...
            '',
            '请使用新密码登录，登录后请及时修改密码以确保账户安全。'
        ]

        if (data.twoFactorDisabled) {
            paragraphs.push('', '⚠️ 重要提示：由于密码重置，您的双因素验证（2FA）已被自动禁用。为了账户安全，建议您重新启用2FA。')
        }
        ...
        const htmlContent = generateEmailHtml({
            ...
            alertType: 'success',
            alertTitle: '密码重置成功',
            alertMessage: '您的账户密码已成功重置。',
            greeting: `您好，${data.username}`,
            paragraphs: paragraphs,
            actionTip: '如果这不是您本人的操作，请立即联系管理员。请妥善保管您的新密码，不要将其透露给任何人。',
            brandName,
        brandLogoUrl
        })
        ...
        const textContent = generateEmailText({
            ...
            greeting: `您好，${data.username}`,
            paragraphs: paragraphs,
            actionTip: '如果这不是您本人的操作，请立即联系管理员。',
            brandName,
        brandLogoUrl
        })
        ...
        await transporter.sendMail({
            from: config.fromName ? `"${config.fromName}" <${config.fromEmail}>` : config.fromEmail,
            to: email,
            subject: formatBrandSubject(brandName, '密码重置成功'),
            text: textContent,
            html: htmlContent
        })
